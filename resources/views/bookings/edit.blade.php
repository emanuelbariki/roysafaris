        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Edit Guest Booking</h4>
                </div>
                <div class="card-body">
                    @if ($errors->any())
                    ...
                    <form method="POST" action="{{ route('bookings.update', $booking) }}">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('group_name') is-invalid @enderror"
                                           ...
                                           value="{{ old('group_name', $booking->group_name) }}"
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select name="nationality" id="nationality"
                                            class="select2 form-control @error('nationality') is-invalid @enderror">
                                        <option value="">Select Nationality</option>
                                        @foreach(Nationality::groupedByRegion() as $region => $nationalities)
                                            <optgroup label="{{ $region }}">
                                                @foreach($nationalities as $nationalityEnum)
                                                    <option
                                                        value="{{ $nationalityEnum->value }}" {{ old('nationality', $booking->nationality) === $nationalityEnum->value ? 'selected' : '' }}>
                                                        {{ $nationalityEnum->getName() }}
                                                    </option>
                                                @endforeach
                                            </optgroup>
                                        @endforeach
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('remarks') is-invalid @enderror"
                                           ...
                                           value="{{ old('remarks', $booking->remarks) }}"
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    ...
                                    <input type="hidden" id="file_owner" name="file_owner"
                                           value="{{ old('file_owner', $booking->file_owner ?? Auth::user()->id) }}">
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="channel_id" onchange="displayAgentCode(this)" id="channel_id"
                                        ...
                                        @foreach ($channels as $channel)
                                            <option
                                                value="{{ $channel->id }}" {{ old('channel_id', $booking->channel_id) == $channel->id ? 'selected' : '' }}>{{ $channel->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('channel_id')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                            ...
                            <!-- Agent Code -->
                            <div class="col-md-2" id="agent_code_div"
                                 style="display: {{ (old('channel_id', $booking->channel_id) == 6) ? 'block' : 'none' }};">
                                <div class="form-group">
                                    <label for="agent_code" class="form-label">Agents</label>
                                    <select name="agent_code" id="agent_code"
                                            class="select2 form-select form-control @error('agent_code') is-invalid @enderror">
                                        <option value="">Choose Agent</option>
                                        @foreach ($agents as $agent)
                                            <option
                                                value="{{ $agent->id }}" {{ old('agent_code', $booking->agent_code) == $agent->id ? 'selected' : '' }}>{{ $agent->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('agent_code')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="datetime-local"
                                           ...
                                           id="arrival_date"
                                           name="arrival_date"
                                           value="{{ old('arrival_date', substr($booking->arrival_date, 0, 16)) }}"
                                           required>
                                    @error('arrival_date')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="pickup_details"
                                            class="select2 form-control @error('pickup_details') is-invalid @enderror"
                                            id="pickup_details">
                                        <option value="">Choose</option>
                                        @foreach ($pickup_dropoff_points as $pickup_dropoff_point)
                                            <option
                                                value="{{ $pickup_dropoff_point->id }}" {{ old('pickup_details', $booking->pickup_details) == $pickup_dropoff_point->id ? 'selected' : '' }}>{{ $pickup_dropoff_point->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="datetime-local"
                                           ...
                                           id="departure_date"
                                           name="departure_date"
                                           value="{{ old('departure_date', substr($booking->departure_date, 0, 16)) }}"
                                           required>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="drop_details"
                                            class="select2 form-control @error('drop_details') is-invalid @enderror"
                                            id="drop_details">
                                        <option value="">Choose</option>
                                        @foreach ($pickup_dropoff_points as $pickup_dropoff_point)
                                            <option
                                                value="{{ $pickup_dropoff_point->id }}" {{ old('drop_details', $booking->drop_details) == $pickup_dropoff_point->id ? 'selected' : '' }}>{{ $pickup_dropoff_point->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('drop_details')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                        ...
                        <div class="services mb-4">
                            @php
                                ...
                                $selectedServices = old('services', json_decode($booking->services, true) ?? []);
                            @endphp
                            <div class="row">
                        ...
                        <div class="row mt-4">
                            <div class="col-sm-12 text-right">
                                ...
                                <button class="btn btn-primary px-4" type="submit">Update Booking</button>
                            </div>
                        </div>
                    </form>
                </div>
...
@push('scripts')
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            // Set initial state of agent code field
