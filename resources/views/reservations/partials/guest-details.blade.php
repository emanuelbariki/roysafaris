...
<div class="mb-4">
    ...
    <div class="row g-3 mt-2">
        ...
        <div class="col-md-3">
            ...
            <input type="text" disabled name="guest_name" id="name" value="{{ $booking->group_name }}" class="form-control @error('guest_name') is-invalid @enderror" required>
            @error('guest_name') <small class="text-danger">{{ $message }}</small> @enderror
        </div>

        <div class="col-md-3 hide">
            <label class="form-label">Agent Name</label>
            <select disabled class="form-select form-control @error('agent_id') is-invalid @enderror" name="agent_id">
                <option value="">Select Agent</option>
                @foreach($agents as $agent)
                    <option value="{{ $agent->id }}" {{ old('agent_id', $booking->agent_id ?? '') == $agent->id ? 'selected' : '' }}>{{ $agent->name }}</option>
                @endforeach
            </select>
