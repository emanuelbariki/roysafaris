...
class HotelChainController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $tData['hotelchain'] = null;
        if ($request->has('edit')) {
            $tData['hotelchain'] = HotelChain::find($request->edit);
        }
        $tData['hotelchains'] = HotelChain::all();

        $tData['title'] = "Hotel Chains";
        return view('masters.hotel_chains', $tData);
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!$exists) {
            try {
                ...
                return redirect()->route('hotelchains.index')->with('error', 'An error occurred while adding the car brand.');
            } 
        }

        return redirect()->route('hotelchains.index')->with('error', 'Record already exists!');
    }

    /**
     * Display the specified resource.
     */
    public function show(HotelChain $fleetType)
    {
        //
    }
     ...
     */
    public function edit(HotelChain $fleetType)
    {
        //
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $validatedData = $request->validate([
            'hotelchain.name' => 'required|string|max:255',
        ]);

        $hotelchain = $validatedData['hotelchain'];
        // $hotelchain['modified_by'] = Auth::id();

        $HotelChain->update($hotelchain);

        return redirect()->route('hotelchains.index')->with('success', 'Record updated successfully!');
    }
     ...
     */
    public function destroy(HotelChain $fleetType)
    {
        //
    }
}
